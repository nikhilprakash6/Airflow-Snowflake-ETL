...
SELECT 
    ...
    'REFRESH_CUSTOMER_TABLE' JOB_NAME,
    2 STEP_NUMBER,
    'Insert Data to STG_CUSTOMER table from DATA_ACCESS Database  and handling the NULL values' STEP_DESC,
    ...
    ------ Handling the NULL -------------
    -- 1. Removing rows which are having NULL values for BIRTHDATE, MARITALSTATUS, GENDER, ANNUALINCOME, TOTALCHILDREN, EDUCATIONLEVEL, OCCUPATION, HOMEOWNER
    DELETE FROM STAGE.MS_ADWBI.STG_CUSTOMER WHERE BIRTHDATE IS NULL AND MARITALSTATUS IS NULL AND GENDER IS NULL AND ANNUALINCOME IS NULL AND TOTALCHILDREN IS NULL AND EDUCATIONLEVEL IS NULL AND OCCUPATION IS NULL AND HOMEOWNER IS NULL;
    
    -- 2. Inserting prefix values, If GENDER is M then MR., else if GENDER is F and TOTALCHILDREN > 0 then MRS. and else MS.
    UPDATE STAGE.MS_ADWBI.STG_CUSTOMER
    SET PREFIX =
        CASE
            WHEN GENDER = 'M' THEN 'MR.'
            WHEN GENDER = 'F' AND TOTALCHILDREN > 0 THEN 'MRS.'
            WHEN GENDER = 'F' AND TOTALCHILDREN = 0 THEN 'MS.'
        END;$$ AS ETL_LOGIC,
...
SELECT 
    ...
    'REFRESH_PRODUCT_TABLE' JOB_NAME,
    2 STEP_NUMBER,
    'Insert Data to STG_PRODUCT table from DATA_ACCESS Database and handling the NULL values' STEP_DESC,
    ...
    ------ Handling the NULL -------------
    UPDATE STAGE.MS_ADWBI.STG_PRODUCT
    SET PRODUCTCOLOR = 'NA'
    WHERE PRODUCTCOLOR IS NULL;
    ...
    UPDATE STAGE.MS_ADWBI.STG_PRODUCT
    SET PRODUCTSTYLE = 0
    WHERE PRODUCTSTYLE IS NULL;
    
    UPDATE STAGE.MS_ADWBI.STG_PRODUCT
    SET
       MODELNAME =
            CASE
                WHEN POSITION(',' IN PRODUCTNAME) > 0
                THEN TRIM(SPLIT_PART(PRODUCTNAME, ',', 1))
                ELSE TRIM(PRODUCTNAME)
            END
    WHERE MODELNAME IS NULL;$$ AS ETL_LOGIC,
    'DATA_ACCESS' SOURCE_DATABASE_NAME,
